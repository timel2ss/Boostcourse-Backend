...
public class ProductImageRepository {
    ...
    public ProductImage findByDisplayId(long displayId) {
        Map<String, ?> parameter = Collections.singletonMap("display_id", displayId);
        return namedParameterJdbcTemplate.queryForObject("select pi.id, pi.product_id, pi.type, pi.file_id from display_info d, product p, product_image pi where d.product_id = p.id and p.id = pi.product_id and d.id = :display_id", parameter, productImageRowMapper());
    }
